...
<%-- 
    ...
    uri(Uniform Resource Identifier, 통합 자원 식별자)
        -> 자원을 식별하는 고유 문자열  (ID) , 뭐라고 부를건데~
        http://java.sun.com/jsp/jstl/core => 이게 주소(url)가 아니라 id같은거다...

    (참고) url(Uniform Resource Locator)
        -> 자원의 위치를 나타내는 문자열 (경로) , 어딧는데~
...
<body> 
    ...
    <pre>
        ...
        표준으로 제공하는 라이브러리
        (if, for 간단히 쓰고싶은(쓰고싶으면?) 이 라이브러리를 사용해라)
    </pre>
    ...
    <% 
        // 스크립틀릿으로 page scope에 속성 세팅하는 방법 -> 원래는 이렇게 써야한다...
        pageContext.setAttribute("num1", 10);
    %>
    
    <%--  html에서 스크립틀릿을 쓸때 이질감이 느껴진다... 그래서 태그형태로~ --%>
