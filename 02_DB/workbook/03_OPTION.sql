...
/*5 5. 2004년 2학기에 'C3118100' 과목을 수강한 학생들의 학점을 조회하려고 한다. 학점이 높은 학생부터
표시하고,
...
RIGHT JOIN TB_PROFESSOR P USING (PROFESSOR_NO)
...
SELECT STUDENT_NO 학번, STUDENT_NAME, ROUND(AVG(POINT), 1) 
FROM TB_DEPARTMENT
...
SELECT S.STUDENT_NO 학번, S.STUDENT_NAME 이름, DEPARTMENT_NAME, AVG(POINT)
FROM TB_STUDENT S
...
--SELECT *
--FROM TB_STUDENT S
--WHERE S.ABSENCE_YN = 'N'; --> 휴학생이 아닌 학생 테이블
...
WHERE D.DEPARTMENT_NAME = '환경조경학과' ; -- 자연과학

-- 저 계열들에 속하는 학과들을 뽑아보자 --> 최종
