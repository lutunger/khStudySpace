/*
    * SUBQUERY (서브쿼리)
    - 하나의 SQL문 안에 포함된 또다른 SQL문(SELECT 문)
    - 메인쿼리(기존쿼리)를 위해 보조 역할을 하는 쿼리문(SELECT문을 QUERY문이라고 자주 말한다)
...
SELECT EMPLOYEE.EMP_ID  ,EMP_NAME, JOB_CODE , DEPT_CODE, SALARY, EMPLOYEE.HIRE_DATE 
FROM EMPLOYEE
WHERE SALARY = (SELECT MIN(SALARY) FROM EMPLOYEE); 

-- 급여가 가장 적은 사람의 사번
-- ROWNUM : 행번호를 출력하는 가상의 컬럼
--			(주의사항!) ORDER BY절 해석 전에 먼저 행번호를 부여 --> 그래서 따로 테이블을 만들었다(서브쿼리)
SELECT EMP_ID FROM 
(SELECT EMP_ID
FROM EMPLOYEE
ORDER BY SALARY)
...
-- 메인쿼리 + 서브쿼리(WHERE) + 서브쿼리(FROM) --> 좀더 정확한 데이터를 얻을 수 있다.
-- (복잡 하지만 신뢰도 있는 정보 조회)
SELECT EMPLOYEE.EMP_ID  ,EMP_NAME, JOB_CODE , DEPT_CODE, SALARY, EMPLOYEE.HIRE_DATE 
FROM EMPLOYEE
...
(SELECT EMP_ID
FROM EMPLOYEE
ORDER BY SALARY, /*HIRE_DATE*/) --> 조건을 덧붙여서 더 세분화 할 수도 있다.
WHERE ROWNUM = 1); 
...
--GROUP 함수 SUM, MIN/MAX, AVG, COUNT
...
-- 선택 함수(DECODE, CASE --> 지금은 CASE로)로 풀이
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_NAME, 
	CASE 	 
...
-- > 220, 250 , 376 중에서 큰값 작은값 중에서 하나 뽑아줌 
--			위에 등호를 써서 작은값을 뽑아준거임 , 부등호가 반대면 큰값 뽑아줌
...
-- 최종
SELECT E.EMP_ID , E.EMP_NAME , E.DEPT_CODE , J.JOB_CODE , E.HIRE_DATE 
FROM EMPLOYEE E
JOIN JOB J ON (E.JOB_CODE = J.JOB_CODE)
JOIN DEPARTMENT D ON (E.DEPT_CODE = D.DEPT_ID)
...
SELECT *
FROM EMPLOYEE E
WHERE SUBSTR(EMP_NO, 8, 1) = '2' -- 여자
AND SUBSTR(EMP_NO, 1, 2) = '77'; -- 77년생

-- 
SELECT E.EMP_ID , E.EMP_NAME , E.DEPT_CODE , E.MANAGER_ID , E.EMP_NO , E.HIRE_DATE 
FROM EMPLOYEE E
WHERE (E.DEPT_CODE , E.MANAGER_ID) = (SELECT E.DEPT_CODE , E.MANAGER_ID 
										FROM EMPLOYEE E
...
-- ANY는 대소 비교만 ... JOB_CODE는 대소가 비교가능한 리터럴이 아니다.
-- 보통 서브쿼리 먼저 메인쿼리 나중에 한다.
...
-- 그냥 서브 쿼리 테이블 이랑 메인 테이블이랑 따로 구분해서 쿼리 돌리겠다 그거잖아..
...
-- NULL을 어떻게 처리할 것인가에 대한 것이 사실 주요문제지 않나... 
...
SELECT E.EMP_ID , E.EMP_NAME , E.MANAGER_ID , 
		NVL((SELECT SE.EMP_NAME 
		 FROM EMPLOYEE SE
		 WHERE SE.EMP_ID  = E.MANAGER_ID), '없음') 관리자명
FROM EMPLOYEE E;
...
/*1*/FROM EMPLOYEE
/*2*/WHERE ROWNUM <= 5
/*4*/ORDER BY SALARY DESC; --> EMPLOYEE 테이블에 5명을 내림차순 한거밖에 안된다.
...
SELECT EMP_NAME, ROWNUM
FROM EMPLOYEE
WHERE ROWNUM = 2; -- 결과값이 안나옴
-- ROWNUM은 가상의 테이블이라서 ROWNUM은 1부터 시작해서 넘버가 붙는데
-- ROWNUM을 2부터 보려고하면 1부터 ROWNUM을 가상으로 붙이지 않기 때문에 결과값이 안나온다
...
---- 부서마다 급여 테이블
--SELECT NVL(E.DEPT_CODE, '부서 없음'), SALARY
--FROM EMPLOYEE E
--LEFT JOIN DEPARTMENT D ON (E.DEPT_CODE = D.DEPT_ID);
--
---- 부서별 급여 평균 테이블
--SELECT DEPT_CODE, NVL(DEPT_TITLE, '부서 없음') , FLOOR(AVG(SALARY))
--FROM (SELECT NVL(E.DEPT_CODE, '부서 없음') AS DEPT_CODE, D.DEPT_TITLE AS DEPT_TITLE ,SALARY
--		FROM EMPLOYEE E
--		LEFT JOIN DEPARTMENT D ON (E.DEPT_CODE = D.DEPT_ID))
--GROUP BY DEPT_CODE, DEPT_TITLE
...
---- 급여 평균 순서 나열
--SELECT *
--FROM (SELECT DEPT_CODE, NVL(DEPT_TITLE, '부서 없음') AS DEPT_TITLE , FLOOR(AVG(SALARY)) AS "AVG_SALARY"
--		FROM (SELECT NVL(E.DEPT_CODE, '부서 없음') AS DEPT_CODE, D.DEPT_TITLE AS DEPT_TITLE ,SALARY
--				FROM EMPLOYEE E
--				LEFT JOIN DEPARTMENT D ON (E.DEPT_CODE = D.DEPT_ID))
--		GROUP BY DEPT_CODE, DEPT_TITLE
