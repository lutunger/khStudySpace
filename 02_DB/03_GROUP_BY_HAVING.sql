...
-- 부서별 급여 합 조회
SELECT DEPT_CODEM SUM(SALARY) FROM EMPLOYEE;
-- ORA-00937 : 단일 그룹의 그룹 함수가 아닙니다

/*3*/SELECT DEPT_CODE, SUM(SALARY) -- 3) 그룹별 SALARY 합계를 조회
/*1*/FROM EMPLOYEE   -- 1) EMPLOYEE 테이블에서
/*2*/GROUP BY DEPT_CODE; -- 2) DEPT_CODE가 같은 행 끼리 그룹을 지어
...
SELECT JOB_CODE, COUNT(NVL(BONUS,NULL)) 
FROM EMPLOYEE
GROUP BY JOB_CODE  
ORDER BY JOB_CODE;
...
SELECT DEPT_CODE , SAL_LEVEL, ROUND(AVG(SALARY)), COUNT(*)
FROM EMPLOYEE
...
SELECT DEPT_CODE, AVG(SALARY)
FROM EMPLOYEE
WHERE EXTRACT (YEAR FROM (TO_DATE(SUBSTR(EMP_NO, 1, 6)))) >= 1970
AND EXTRACT (YEAR FROM (TO_DATE(SUBSTR(EMP_NO, 1, 6)))) < 1980
...
-- CUBE : 그룹으로 지정된 모든 그룹에 대한 중간 집계와 총합
SELECT DEPT_CODE , JOB_CODE , SUM(SALARY)
FROM EMPLOYEE
...
-- EX) 신고 모아보기(신고-> 게시물 신고 테이블, 댓글 신고 테이블, 사용자 신고 테이블) --> 유용함...
