...
SELECT EMP_NAME "[사원 이름]", SALARY * 12 연봉
FROM EMPLOYEE;
...
-- NOT IN(값, 값2, 값3, ....) : ()내 값을 제외한 행 TRUE
SELECT EMP_ID 사번, EMP_NAME 이름, DEPT_CODE 부서코드
FROM EMPLOYEE
...
-- EMPLOYEE 테이블에서 부서가 없는 사원의 모든 컴럼 조회
SELECT * 
FROM EMPLOYEE
...
SELECT EMP_ID 사번, EMP_NAME 이름 -- 3
FROM EMPLOYEE -- 1
...
-- 보통 $ , # 쓴다    --> '#' 뒤에 한 글자(_)를 일반 문자로 벗어나게함
...
FROM EMPLOYEE
WHERE HIRE_DATE >= '1990/01/01' --'1990-01-01'
AND   HIRE_DATE <= '2000/12/31'; -- '2000-12-31'

--SELECT EMP_ID, EMP_NAME, HIRE_DATE
--FROM EMPLOYEE
--WHERE HIRE_DATE BETWEEN '1990/01/01' AND '2000/12/31';
--SELECT * FROM EMPLOYEE;
--> self wirte code above

--> '1990/01/01' == 문자열
--> 오라클 DB 는 작성된 값이 다른 형식의 데이터 타입이여도
