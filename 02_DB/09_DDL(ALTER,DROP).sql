...
ALTER TABLE DEPT_COPY ADD
CONSTRAINT UK_DEPT_TITLE UNIQUE(DEPT_TITLE);
...
MODIFY DEPT_TITLE CONSTRAINT SYS_C007547 /*NOT*/ NULL; 
										--> /**/ 실행하면 DROP하는거랑 같다.
...
-- * DDL / DML을 혼용해서 사용할 경우 발생하는 문제점
	-- DDL : 데이터 정의(DEFINITION) 언어(객체를 CREATE, ALTER, DROP)
	-- DML : 데이터 조작(MANIPULATION) 언어(테이블에 값을 INSERT, UPDATE, DELETE)
	-- TCL : 트랜잭션(TRANSACTION) 제어(CONTROL) 언어(DML 수행 내용을 COMMIT, ROLLBACK, SAVEPOINT)
...
-- DROP TABLE 테이블명 [CASCADE CONSTRAINTS];
						--> CASCADE 종속? ===> 종속 제약조건까지 지울래~
						--> FK 제약조건이 있으면 테이블이 안지워지기 때문에
						--> FK 제약조건을 없애기 위해 써야한다.
