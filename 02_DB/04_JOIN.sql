...
-- '='(등호, EQUAL)를 사용하지 않는 조인문
--  지정한 컬럼 값이 일치하는 경우가 아닌, 값의 범위에 포함되는 행들을 연결하는 방식

SELECT EMP_NAME, SALARY, E.SAL_LEVEL, S.SAL_LEVEL
FROM EMPLOYEE E
JOIN SAL_GRADE S ON ( SALARY BETWEEN MIN_SAL AND MAX_SAL);                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      LARY BETWEEN MIN_SAL AND MAX_SAL );
...
--연봉에 보너스포인트를 적용하시오.
SELECT E.EMP_NAME 사원명, J.JOB_NAME 직급명, SALARY 급여, (SALARY*(NVL(BONUS,0)+1) * 12) "연봉(보너스포함)"
FROM EMPLOYEE E
...
SELECT EMP_NAME 사원명, D.DEPT_TITLE 부서명, L.LOCAL_NAME 지역명, N.NATIONAL_NAME 국가명
FROM EMPLOYEE E
JOIN JOB J USING(JOB_CODE)
LEFT JOIN DEPARTMENT D ON (D.DEPT_ID = E.DEPT_CODE)
LEFT JOIN LOCATION L  ON (LOCATION_ID = LOCAL_CODE)
...
-- SELF JOIN 사용
SELECT ONE.EMP_NAME 사원명, ONE.DEPT_CODE 부서코드, TWO.EMP_NAME 사원명 
FROM EMPLOYEE ONE, EMPLOYEE TWO
WHERE ONE.DEPT_CODE(+) = TWO.DEPT_CODE
AND ONE.EMP_NAME != TWO.EMP_NAME;

-- 9. 보너스포인트(컬럼)가 없는 직원들 중에서 직급코드가 J4와 J7인 직원들의 사원명, 직급명, 급여를 조회하시오.
-- 단, JOIN, IN 사용할 것
SELECT EMP_NAME 사원명, J.JOB_NAME 직급명, SALARY 급여
FROM EMPLOYEE E
JOIN JOB J ON (E.JOB_CODE = J.JOB_CODE)
LEFT JOIN DEPARTMENT D ON (D.DEPT_ID = E.DEPT_CODE)
LEFT JOIN LOCATION L  ON (LOCATION_ID = LOCAL_CODE)
