...
public class JDBCExample1 {
	public static void main(String[] args) {
		...
		/*
		 * ** JDBC를 이용한 애플리케이션 제작 시 필요한 것 ** --> 전제조건, setup
		 * 
		 * 1. Java의 JDBC 관련 인터페이스 2. DBMS(Data Base Management System)의 종류 - 우리는 ORACLE
		 * 3. DBMS 회사에서 제공하는 Java 애플리케이션과 DB 연결을 위한 라이브러리 (ojdbc11.jar ->
		 * OracleDriver.class (제공))
		 */

		// 1단계 : JDBC 객체 참조 변수 선언
//					--> 보통 인터페이스로 되어 있기때문에(추상화된 클래스) 참조 변수로만 접근한다

		Connection conn = null;
		// DB 연결 정보를 담는 객체 - 보통 Connection이라고 한다.
...
//		Initially the cursor is positionedbefore the first row. 
...
//		it can be used in a while loop to iterate throughthe result set. 
		...
		try {
			...
			String user = "kh_kjh"; // 계정명

			String pw = "oracle_kjh123A"; // 비밀번호
...
//			System.out.println(conn);
			...
			while (rs.next()) {
				...
				// 현재 커서가 접근한 행의 "EMP_ID" 컬럼의 값을얻어와
				// String으로 반환

				String empName = rs.getString("EMP_NAME");
				// 현재 커서가 접근한 행의 "EMP_NAME" 컬럼의 값을얻어와
				// String으로 반환

				int salary = rs.getInt("SALARY");
				// 현재 커서가 접근한 행의 "SALARY" 컬럼의 값을얻어와
				// int로 반환
				...
				// 현재 커서가 접근한 행의 "HIRE_DATE" 컬럼의 값을얻어와
				// Date클래스로 반환
				...
				System.out.printf("사번: %s / 이름: %s / 급여: %s /  입사일: %s \n", empId, empName, salary,
						hireDate.toString());
