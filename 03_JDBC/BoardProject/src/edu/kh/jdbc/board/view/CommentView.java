...
public class CommentView {
	...
	private void insertComment(int boardNo) {
		...
		System.out.println("등록할 댓글 내용을 입력하세요 <!wq 입력 시 종료>");
		...
		try {
			...
			int result = service.insertComment(boardNo, sb.toString(), 
	...
	private void updateComment(int boardNo) {
		...
		int matchCommentWriter = 0; 
		
		// 1은 일치 0은 불일치 
		try {
			matchCommentWriter = service.matchCommentWriter(
		...
		if(matchCommentWriter == 0) {
			...
			System.out.println("수정할 댓글 내용을 입력하세요 <!wq 입력 시 종료>");
